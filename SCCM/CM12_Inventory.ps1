...
.EXAMPLE
	PS C:\PSScript > .\CM12_Inventory.ps1 -ManagementPoint CM12.do.local -SiteCode PR1
    ...
    CM12.do.local for the ManagementPoint.
    PR1 for the Site Code.
.EXAMPLE
	PS C:\PSScript > .\CM12_Inventory.ps1 -ManagementPoint CM12.do.local -SiteCode PR1 -ListAllInformation
...
.EXAMPLE
	PS C:\PSScript > .\CM12_Inventory.ps1 -ManagementPoint CM12.do.local -SiteCode PR1 -verbose
...
.EXAMPLE
	PS C:\PSScript > .\CM12_Inventory.ps1 -ManagementPoint CM12.do.local -SiteCode PR1 -CompanyName "David's company" -CoverPage "Motion" -UserName "David O'Brien"
...
.NOTES
	...
	LASTEDIT: June 22, 2013
    Change history:
        19.06.2013: added error checks, bitness of powershell process, has powershell module been loaded?
		22.06.2013: added more error checks (Try...Catch)
.REMARKS
...
#check Bitness script is running in
...
$Title="Inventory Report for Customer $($CompanyName)"
    ...
    foreach ($CMDistributionPoint in $CMDistributionPoints)
        {
            ...
            WriteWordLine 0 1 "$($CMDPServerName)"
            #$DPInfo = Get-WmiObject -Class SMS_DistributionPointInfo -Namespace root\sms\site_$($CMSite.SiteCode) -ComputerName $CMMPServerName | Where-Object {$_.Name -eq "$($CMDPServerName)"}
            ...
            #WriteWordLine 0 1 "This is a protected Distribution Point: $($DPInfo.IsProtected)"
            #WriteWordLine 0 1 "This is a Pull Distribution Point: $($DPInfo.IsPullDP)"
            $DPGroupMembers = $Null
            $DPGroupIDs = $Null
            $DPGroupMembers = Get-WmiObject -class SMS_DPGroupMembers -Namespace root\sms\site_$SiteCode -ComputerName $ManagementPoint | Where-Object {$_.DPNALPath -ilike "*$($CMDPServerName)*"}
        ...
        foreach ($BoundaryGroup in $BoundaryGroups)
            {
                ...
                if ($BoundaryGroup.SiteSystemCount -gt 0)
                    {
                        $MemberIDs = (Get-WmiObject -Class SMS_BoundaryGroupMembers -Namespace root\sms\site_$SiteCode -ComputerName $ManagementPoint | Where-Object {$_.GroupID -eq "$($BoundaryGroup.GroupID)"}).BoundaryID
...
foreach ($ClientSetting in $AllClientSettings)
    {
        ...
        WriteWordLine 0 2 "Configurations:"
                        ...
                        2
                            {
                                ...
                                if ($AgentConfig.ExcludeWindirAndSubfolders -eq "true")
                                    {
        ...
        foreach ($DeviceCollection in $DeviceCollections)
            {
                ...
                $CollSettings = Get-WmiObject -Class SMS_CollectionSettings -Namespace root\sms\site_$SiteCode -ComputerName $ManagementPoint | Where-Object {$_.CollectionID -eq "$($DeviceCollection.CollectionID)"}
        ...
        foreach ($AntiMalwarePolicy in $AntiMalwarePolicies)
            {
                ...
                if ($AgentConfig.EnableScheduledScan)
                    {
                        ...
                        WriteWordLine 0 2 "Scan type: $($ScheduledScanType)"
                        WriteWordLine 0 2 "Scan day: $(Convert-WeekDay $AgentConfig.ScheduledScanWeekDay)"
                        WriteWordLine 0 2 "Scan time: $(Convert-Time -time $AgentConfig.ScheduledScanTime)"
                        WriteWordLine 0 2 "Run a daily quick scan on client computers: $($AgentConfig.EnableQuickDailyScan)"
                        WriteWordLine 0 2 "Daily quick scan schedule time: $(Convert-Time -time $AgentConfig.ScheduledScanQuickTime)"
                        WriteWordLine 0 2 "Check for the latest definition updates before running a scan: $($AgentConfig.CheckLatestDefinition)"
                        WriteWordLine 0 2 "Start a scheduled scan only when the computer is idle: $($AgentConfig.ScanWhenClientNotInUse)"
                        WriteWordLine 0 2 "Force a scan of the selected scan type if client computer is offline during two or more scheduled scans: $($AgentConfig.EnableCatchupScan)"
                        WriteWordLine 0 2 "Limit CPU usage during scans to (%): $($AgentConfig.LimitCPUUsage)"
                    }
                WriteWordLine 0 0 ""
                WriteWordLine 0 2 "Scan settings" -bold
                WriteWordLine 0 2 "Scan email and email attachments: $($AgentConfig.ScanEmail)"
                WriteWordLine 0 2 "Scan removable storage devices such as USB drives: $($AgentConfig.ScanRemovableStorage)"
                WriteWordLine 0 2 "Scan network drives when running a full scan: $($AgentConfig.ScanNetworkDrives)"
                WriteWordLine 0 2 "Scan archived files: $($AgentConfig.ScanArchivedFiles)"
                WriteWordLine 0 2 "Allow users to configure CPU usage during scans: $($AgentConfig.AllowClientUserConfigLimitCPUUsage)"
                WriteWordLine 0 2 "User control of scheduled scans: " -nonewline
                ...
                WriteWordLine 0 2 "Severe threats: " -nonewline
                ...
                WriteWordLine 0 2 "High threats: " -nonewline
                switch ($AgentConfig.DefaultActionSevere)
                    {
                        0 { WriteWordLine 0 0 "Recommended" }
                        2 { WriteWordLine 0 0 "Quarantine" }
                ...
                WriteWordLine 0 2 "Medium threats: " -nonewline
                switch ($AgentConfig.DefaultActionSevere)
                    {
                        0 { WriteWordLine 0 0 "Recommended" }
                        2 { WriteWordLine 0 0 "Quarantine" }
                ...
                WriteWordLine 0 2 "Low threats: " -nonewline
                switch ($AgentConfig.DefaultActionSevere)
                    {
                        0 { WriteWordLine 0 0 "Recommended" }
                        2 { WriteWordLine 0 0 "Quarantine" }
                ...
                WriteWordLine 0 2 "Enable real-time protection: $($AgentConfig.RealtimeProtectionOn)"
                WriteWordLine 0 2 "Monitor file and program activity on your computer: $($AgentConfig.MonitorFileProgramActivity)"
                WriteWordLine 0 2 "Scan system files: " -nonewline
                ...
                WriteWordLine 0 2 "Excluded files and folders: "
                foreach ($ExcludedFileFolder in $AgentConfig.ExcludedFilePaths)
                    {
                        WriteWordLine 0 3 "$($ExcludedFileFolder)"
                    }
                WriteWordLine 0 2 "Excluded file types: "
                foreach ($ExcludedFileType in $AgentConfig.ExcludedFileTypes)
                    {
                        WriteWordLine 0 3 "$($ExcludedFileType)"
                    }
                WriteWordLine 0 2 "Excluded processes: "
                foreach ($ExcludedProcess in $AgentConfig.ExcludedProcesses)
                    {
                        WriteWordLine 0 3 "$($ExcludedProcess)"
                    }
                WriteWordLine 0 2 "Advanced" -bold
                WriteWordLine 0 2 "Create a system restore point before computers are cleaned: $($AgentConfig.CreateSystemRestorePointBeforeClean)"
                WriteWordLine 0 2 "Disable the client user interface: $($AgentConfig.DisableClientUI)"
                WriteWordLine 0 2 "Show notifications messages on the client computer when the user needs to run a full scan, update definitions, or run Windows Defender Offline: $($AgentConfig.ShowNotificationMessages)"
                WriteWordLine 0 2 "Delete quarantined files after (days): $($AgentConfig.DeleteQuarantinedFilesPeriod)"
                WriteWordLine 0 2 "Allow users to configure the setting for quarantined file deletion: $($AgentConfig.AllowUserConfigQuarantinedFileDeletionPeriod)"
                WriteWordLine 0 2 "Allow users to exclude file and folders, file types and processes: $($AgentConfig.AllowUserAddExcludes)"
                WriteWordLine 0 2 "Allow all users to view the full History results: $($AgentConfig.AllowUserViewHistory)"
                WriteWordLine 0 2 "Enable reparse point scanning: $($AgentConfig.EnableReparsePointScanning)"
                WriteWordLine 0 2 "Randomize scheduled scan and definition update start time (within 30 minutes): $($AgentConfig.RandomizeScheduledScanStartTime)"
        
                WriteWordLine 0 2 "Threat overrides" -bold
                if (-not [string]::IsNullOrEmpty($AgentConfig.ThreatName))
                    {
                        WriteWordLine 0 2 "Threat name and override action: Threats specified."
                    }
                WriteWordLine 0 2 "Microsoft Active Protection Service" -bold
                WriteWordLine 0 2 "Microsoft Active Protection Service membership type: " -nonewline
                ...
                WriteWordLine 0 2 "Allow users to modify Microsoft Active Protection Service settings: $($AgentConfig.AllowUserChangeSpyNetSettings)"

                WriteWordLine 0 2 "Definition Updates" -bold
                WriteWordLine 0 2 "Check for Endpoint Protection definitions at a specific interval (hours): (0 disable check on interval) $($AgentConfig.SignatureUpdateInterval)"
                WriteWordLine 0 2 "Check for Endpoint Protection definitions daily at: (Only configurable if interval-based check is disabled) $(Convert-Time -time $AgentConfig.SignatureUpdateTime)"
                WriteWordLine 0 2 "Force a definition update if the client computer is offline for more than two consecutive scheduled updates: $($AgentConfig.EnableSignatureUpdateCatchupInterval)"
                WriteWordLine 0 2 "Set sources and order for Endpoint Protection definition updates: "
                foreach ($Fallback in $AgentConfig.FallbackOrder)
                    {
                        WriteWordLine 0 3 "$($Fallback)"
                    }
                WriteWordLine 0 2 "If Configuration Manager is used as a source for definition updates, clients will only update from alternative sources if definition is older than (hours): $($AgentConfig.AuGracePeriod / 60)"
                WriteWordLine 0 2 "If UNC file shares are selected as a definition update source, specify the UNC paths:" 
                foreach ($UNCShare in $AgentConfig.DefinitionUpdateFileSharesSources)
                    {
                        WriteWordLine 0 3 "$($UNCShare)"
                    }
            }
        }
    else
