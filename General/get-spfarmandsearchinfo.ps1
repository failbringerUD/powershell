<# =====================================================================
## Title       : Get-SPFarmAndSearchinfo.ps1
## Description : This script will collect information regarding the Farm, Search, and the SSA's in the Farm.
## Authors      : Anthony Casillas | Brian Pendergrass | Josh Roark | PG
## Date        : 05-02-2018
## Input       : 
## Output      : 
## Usage       : .\Get-SPFarmAndSearchinfo.ps1
## Notes       : 
...
## Change log  : 1.2 [acasilla]
##             :   - Added logic to support more than 1 SSA through menu selection
##             :   - Altered the Max Docs per index Partition for 2016 since we support 20 million per index partition now
##             :   - Added some messaging to inform what stage of the script we are in.
## Change log  : 1.3 [acasilla]
##             :   - fixed a few typo's
##             :   - integrated some PowerShell that Josh Roark wrote to check Farm Level Timer Service Instances
## Change log  : 1.4 [acasilla]
##             :   - fixed a few typo's
##             :   - we used to have 3 seperate scripts for 2010, 2013 and 2016. Decided to combine them down to 1
## Change log  : 1.5 [acasilla]
##             :   - fixed a few typo's
##             :   - added the writing of the output of VerifyServiceEndPoints data to the file.. there was a SSP Job Control check that would only write to the console
##             :   - fixed how we collect AAMs so we get ALL AlternateUrls and not just web app based
##             :   - Wrote a check to pull TimerJobHistory table row count..
## =====================================================================
#>
...
Write-Output "If you have no SSA or a single SSA, this will move along with no interaction"
...
$outputfilePrefix = $output + "\SPFarmAndSearchInfo_"
...
function GetSSA
{
    ...
    {
        Write-Host ""
        Write-Host "There is no SSA in this farm. We will still collect some basic informtation"
        Write-Host ""
        return NoSSA
    }
...
{
   Write-Host "Getting Service Application Information that are in an 'Online' status"
    ""
    "#########################################################################################"
    ...
    $serviceApps = Get-SPServiceApplication |?{$_.Status -ne "Disabled"}
      "DisplayName"  + " -- "  + "TypeName"  + " -- "  + "Id" + " -- "  + "Status"
    ""
    foreach ($spserviceApp in $serviceApps)
    {
        $spserviceApp.DisplayName + " -- " + $spserviceApp.TypeName + " -- " + $spserviceApp.Id.ToString() + " -- " + $spserviceApp.Status
    } 
}
...
function CheckTimerServiceInstances()
{
    ...
    Write-Host "Checking Timer Service Instances at the Farm Level. Disabled Service Instances can prevent timer jobs from executing... "
    ""
    "#########################################################################################"
    "  Farm Level Timer Service Instances "
    "#########################################################################################"
...
{
    Write-Host ""
    Write-Host "Checking the size of the 'timerjobhistory' table. If there are millions of rows, this can prevent or cause timer jobs to time out. "
...
function CheckFarmSiteSubscriptions
{
    ...
    {
        Write-Host("We have detected SiteSubscriptions in this farm. If your site contains subscriptionId's and the SSA is not not Partitioned (' This is not common' ), then search results and usage\analytics can be impacted by this.") -ForegroundColor Cyan
        Write-Host  ""
        ""
        "#########################################################################################"
...
function VerifyApplicationServerSyncJobsEnabled
{
	...
	if ($sspJob.Status -ne "Online") { 
        ...
        ""
        Write-Output ("SSP Job Control Service is " + $sspJob.Status)
        ""
		$global:serviceDegraded = $true
	}

	$serverNames = $((Get-SPFarm).Servers | Where {$_.Role -eq "Application" -OR $_.Role -eq "ApplicationWithSearch" -OR $_.Role -eq "Custom" -OR $_.Role -eq "Search" -OR $_.Role -eq "SingleServerFarm"}).Name
	foreach ($server in $serverNames) 
    {
		...
		if ($sspJobServiceInstance.Status -ne "Online") { 
            ...
            ""
            Write-Output ("SSP Job Control Service Instance is " + $sspJobServiceInstance.Status + " on " + $server)
            ""
...
function GetSPVersion()
{
    If($farm.BuildVersion.Major -eq 16 -or $farm.BuildVersion.Major -eq 15)
    {
        ...
        if($farm.BuildVersion.Major -eq 15)
        {
            $is2013 = $true
    ...
    If($farm.BuildVersion.Major -eq 14)
    {
        ...
        "" | Out-File $outputfile -Append
        "" | Out-File $outputfile -Append
        displayGlobalSearchService | ft -auto | Out-File $outputfile -Append
