...
} until(Test-Connection $TemplateVMName -Quiet | Where-Object { $True } )
...
$password=Get-Content TemplatePass.txt | ConvertTo-SecureString
...
Invoke-VMScript -ScriptType PowerShell -ScriptText "import-module PSWindowsUpdate; Get-WUInstall –AcceptAll –AutoReboot -Verbose | Out-File C:\PSWindowsUpdate.log -Append" -VM $TemplateVMName -GuestCredential $GuestOSCred #| Out-file -Filepath C:\WUResults.log -Append
...
<# Removing this block to replace with a different methodology
do
{
  Write-Output "Waiting for $TemplateVMName to respond...`r`n"
  Start-Sleep -Seconds 10
